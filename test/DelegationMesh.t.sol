...
import {DelegationMesh} from "src/DelegationMesh.sol";

contract DelegationMeshTest is Test {
    DelegationMesh private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_metamask_delegations");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new DelegationMesh(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        DelegationMesh freshController = new DelegationMesh(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
