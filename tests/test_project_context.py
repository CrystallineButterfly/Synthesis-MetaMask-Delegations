...
MODULE = importlib.import_module('agents.delegation_mesh')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'delegation_mesh')
        self.assertEqual(spec.primary_contract_name, 'DelegationMesh')
